...
def component_spec_path(cid):
    return ROOT / f"profiles/components/{cid}" / MODULES[cid].get("electrical_profile", "component.json")
...
def wiring_for(ids: list[str]) -> list[dict]:
    ...
    for cid in ids:
        module = MODULES[cid]
        component = json.loads(component_spec_path(cid).read_text(encoding="utf-8"))
...
def profile_versions(ids):
    ...
    paths.update({cid: component_spec_path(cid) for cid in ids})
    result = {}
...
def render_code(wiring: list[dict], params: dict, logic: str, unresolved: list[str]) -> str:
    ...
    if any(w["componentId"] == "hc-sr04" for w in wiring):
        preamble += "# HC-SR04+ wide-voltage variant: VCC = Pi physical Pin 1 (3.3V), ECHO = GPIO18.\n"
        preamble += "# Requires confirmed 3.3V ECHO compatibility; not the standard 5V HC-SR04.\n"
    ...
    if "mrd-tf240-8p-cs" in pins:
        # Keep the deployed program self-contained, not dependent on this repo.
        display_source = (Path(__file__).parent / "runtime/ili9341_display.py").read_text(encoding="utf-8")
        sensor_source = '''
...
with ExitStack() as resources:
    ...
    display = WiringDisplay(PINS["mrd-tf240-8p-cs"], factory)
    resources.callback(display.close)
    display.test_card()
    print("DISPLAY TEST SENT: check RGB bars and text on the physical screen; NOT an automatic pass", flush=True)
    sleep(3)
'''
        if "hc-sr04" in pins:
            body += '''    sensor = resources.enter_context(FreshDistanceSensor(
        echo=PINS["hc-sr04"]["ECHO"], trigger=PINS["hc-sr04"]["TRIG"],
        queue_len=1, partial=True, max_distance=4, pin_factory=factory))
    while True:
        sample = sensor.latest
        distance = None
        if sample is not None and monotonic() - sample[0] <= 1 and sample[1] is not None and 0 < sample[1] < 1:
            distance = round(sample[1] * sensor.max_distance * 100, 1)
            print(namespace["on_sample"]({"distance_cm": distance}, SETTINGS), flush=True)
        else:
            print("distance_cm=UNAVAILABLE (no fresh echo / out of range)", flush=True)
        display.show_distance(distance, SETTINGS["distance_cm"])
        sleep(SETTINGS["sample_ms"] / 1000)
'''
        else:
            body += '''    # Display-only projects keep the test card; never invent a sensor reading.
    while True:
        sleep(1)
'''
        return (preamble + "# TFT: proposed VCC=3.3V (Pin17); verify module rating before power-on. BLK=NC.\n"
                + "from time import sleep, monotonic\nfrom contextlib import ExitStack\n"
                + "from gpiozero.pins.lgpio import LGPIOFactory\n"
                + ("from gpiozero import DistanceSensor\n" + sensor_source if "hc-sr04" in pins else "")
                + display_source + body)
...
def compile_design(proposal: DesignProposal, prompt: str, *, source="ai", current: dict | None = None) -> dict:
    ...
    has_divider = any(w["connectionKind"] == "divider" for w in wiring)
    bom += [{"id": "jumper-wires", "name": "杜邦線 / Jumper wires", "quantity": len(wiring) + (3 if has_divider else 0), "price": 2, "purpose": "模組連接 / Connections"}]
    if has_divider:
...
def demo_design(ids=None):
    ...
    return compile_design(DesignProposal(
        ...
        instructions=["關閉電源後，依逐腳引導接線。"] + (["HC-SR04+：VCC 接 Pi Pin 1（3.3V），ECHO 接 GPIO18；限 3.3V 相容版。"] if has_sensor else [])
        + ([] if sensor_only else ["MRD_TFT240_8P_CS：核對 3.3V 相容性後，VCC 接 Pin 17；BLK 留空。", "Pi 啟用 SPI，安裝目錄指定的 ILI9341 驅動套件。"])
        + ["核對接線後接上 Pi USB-C 電源，待開機，再按連線 Pi 與部署。"],
