...
def _correct_pi5_j8_from_image(
    ...
    """Snap Raspberry Pi 5 GPIO markers to the visible J8 socket body.

    The four board-corner model is good at finding the Pi 5, but the right I/O
    stack can tug the board quad toward the metal USB/Ethernet shells.  GPIO
    teaching cares about the black 2x20 J8 connector, so use the projected
    lattice only as a search prior, then re-center the 40 markers on the
    dark header body that is actually visible in the frame.

    This is intentionally a small, local correction.  If the header cannot be
    found confidently, the caller receives the original projected pins.
    """
    if (
        frame_bgr is None
        or frame_bgr.size == 0
    ...
    j8 = [
        pin for pin in pins
        if getattr(pin, "header", None) == "J8"
        and getattr(pin, "index", None) is not None
        and bool(getattr(pin, "visible", True))
    ]
    if len(j8) < 12:
        return pins

    by_index = {int(pin.index): pin for pin in j8}
    column_centres: list[np.ndarray] = []
    row_vectors: list[np.ndarray] = []
    for col in range(20):
        odd = by_index.get(col * 2 + 1)
        even = by_index.get(col * 2 + 2)
        if odd is None or even is None:
            continue
        odd_xy = np.asarray([odd.x, odd.y], dtype=np.float64)
        even_xy = np.asarray([even.x, even.y], dtype=np.float64)
        column_centres.append((odd_xy + even_xy) * 0.5)
        row_vectors.append(even_xy - odd_xy)
    if len(column_centres) < 6 or not row_vectors:
        return pins

    centres = np.asarray(column_centres, dtype=np.float64)
    long_vec = centres[-1] - centres[0]
    long_norm = float(np.linalg.norm(long_vec))
    if not np.isfinite(long_norm) or long_norm < 30.0:
        return pins
    long_axis = long_vec / long_norm
    row_vec = np.median(np.asarray(row_vectors, dtype=np.float64), axis=0)
    row_norm = float(np.linalg.norm(row_vec))
    if not np.isfinite(row_norm) or row_norm < 4.0:
        return pins
    row_axis = row_vec / row_norm
    if abs(float(np.dot(long_axis, row_axis))) > 0.55:
        return pins

    origin = centres.mean(axis=0)
    projected_xy = np.asarray([[pin.x, pin.y] for pin in j8], dtype=np.float64)
    projected_u = (projected_xy - origin) @ long_axis
    projected_v = (projected_xy - origin) @ row_axis
    projected_pitch = long_norm / 19.0
    if not np.isfinite(projected_pitch) or projected_pitch < 4.0:
        return pins

    search_u_min = float(np.min(projected_u) - 1.2 * projected_pitch)
    search_u_max = float(np.max(projected_u) + 1.2 * projected_pitch)
    search_v_min = float(np.min(projected_v) - 2.2 * row_norm)
    search_v_max = float(np.max(projected_v) + 2.2 * row_norm)
    search_corners = np.asarray([
        origin + search_u_min * long_axis + search_v_min * row_axis,
        origin + search_u_max * long_axis + search_v_min * row_axis,
        origin + search_u_max * long_axis + search_v_max * row_axis,
        origin + search_u_min * long_axis + search_v_max * row_axis,
    ], dtype=np.float64)

    frame_w, frame_h = video_size
    x1 = max(0, int(np.floor(np.min(search_corners[:, 0]))) - 4)
    y1 = max(0, int(np.floor(np.min(search_corners[:, 1]))) - 4)
    x2 = min(frame_w, int(np.ceil(np.max(search_corners[:, 0]))) + 5)
    y2 = min(frame_h, int(np.ceil(np.max(search_corners[:, 1]))) + 5)
    ...
    # Black plastic remains low-value across warm/cool lighting.  Keep the
    # threshold permissive because the socket often contains specular flecks,
    # but gate later by shape and expected direction.
    dark = cv2.inRange(
        hsv,
        np.array([0, 0, 0], dtype=np.uint8),
        np.array([179, 255, 105], dtype=np.uint8),
    )
    polygon = np.round(search_corners - np.asarray([x1, y1], dtype=np.float64)).astype(np.int32)
    poly_mask = np.zeros(dark.shape, dtype=np.uint8)
    cv2.fillConvexPoly(poly_mask, polygon, 255)
    dark = cv2.bitwise_and(dark, poly_mask)
    dark = cv2.morphologyEx(dark, cv2.MORPH_CLOSE, np.ones((5, 5), dtype=np.uint8))
    dark = cv2.morphologyEx(dark, cv2.MORPH_OPEN, np.ones((3, 3), dtype=np.uint8))

    contours, _ = cv2.findContours(dark, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    candidates: list[tuple[float, float, float, float, float, float, np.ndarray]] = []
    expected_centre_uv = np.asarray([
        float((np.min(projected_u) + np.max(projected_u)) * 0.5),
        float((np.min(projected_v) + np.max(projected_v)) * 0.5),
    ], dtype=np.float64)
    for contour in contours:
        area = float(cv2.contourArea(contour))
        if area < max(60.0, projected_pitch * row_norm * 1.5):
            continue
        points = contour.reshape(-1, 2).astype(np.float64) + np.asarray([x1, y1])
        if points.shape[0] < 8:
            continue
        local = np.column_stack(((points - origin) @ long_axis, (points - origin) @ row_axis))
        u_low, u_high = np.percentile(local[:, 0], [2.0, 98.0])
        v_low, v_high = np.percentile(local[:, 1], [5.0, 95.0])
        u_span = float(u_high - u_low)
        v_span = float(v_high - v_low)
        if u_span < 8.0 * projected_pitch or v_span < 0.8 * row_norm:
            continue
        if u_span / max(v_span, 1.0) < 4.0:
            continue
        centre_uv = np.asarray([(u_low + u_high) * 0.5, (v_low + v_high) * 0.5])
        centre_error = float(np.linalg.norm(centre_uv - expected_centre_uv))
        if centre_error > max(3.5 * projected_pitch, 2.5 * row_norm):
            continue
        candidates.append((area, -centre_error, u_low, u_high, v_low, v_high, points))
    if not candidates:
        return pins

    _area, _neg_error, u_low, u_high, v_low, v_high, _points = max(
        candidates, key=lambda item: (item[0], item[1])
    )
    body_length = float(u_high - u_low)
    body_width = float(v_high - v_low)
    if body_length < 8.0 * projected_pitch or body_width < row_norm:
        return pins

    corrected_pitch = body_length / 20.0
    if not (0.55 * projected_pitch <= corrected_pitch <= 1.55 * projected_pitch):
        return pins
    corrected_row_sep = row_norm
    # Plastic, shadows and inserted leads change the outer body's width;
    # they cannot change the physical two-row spacing. Keep the profile's
    # perspective spacing. The old width threshold abruptly doubled it.
    v_centre = float((v_low + v_high) * 0.5)
    odd_mean_v = float(np.mean([
        projected_v[i] for i, pin in enumerate(j8)
        if int(pin.index) % 2 == 1
    ]))
    even_mean_v = float(np.mean([
        projected_v[i] for i, pin in enumerate(j8)
        if int(pin.index) % 2 == 0
    ]))
    odd_v = v_centre - 0.5 * corrected_row_sep
    even_v = v_centre + 0.5 * corrected_row_sep
    if even_mean_v < odd_mean_v:
        odd_v, even_v = even_v, odd_v

    corrected_by_id: dict[str, np.ndarray] = {}
    for pin in j8:
        index = int(pin.index)
        column = (index - 1) // 2
        u = float(u_low + (column + 0.5) * corrected_pitch)
        v = even_v if index % 2 == 0 else odd_v
        corrected_by_id[pin.pin_id] = origin + u * long_axis + v * row_axis

    original_by_id = {
        pin.pin_id: np.asarray([pin.x, pin.y], dtype=np.float64)
        for pin in j8
    }
    shifts = [
        float(np.linalg.norm(corrected_by_id[pin_id] - original))
        for pin_id, original in original_by_id.items()
        if pin_id in corrected_by_id
    ]
    if not shifts or float(np.median(shifts)) > max(3.0 * projected_pitch, 48.0):
        return pins

    corrected: list[PinDetection] = []
    for pin in pins:
        point = corrected_by_id.get(pin.pin_id)
        if point is None:
            corrected.append(pin)
            continue
        x, y = float(point[0]), float(point[1])
        corrected.append(PinDetection(
            pin_id=pin.pin_id,
            x=x,
            y=y,
