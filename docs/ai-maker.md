...
2026-09-22 更新：上述舊版操作說明中的兩種對話模式已整合成單一輸入框，不再手動選「詢問 AI／設計修改」；現行零件、流程與驗證見 [統一對話與 Demo](unified-maker-chat.md)。
直接提問只回覆文字；明確要求設計或修改才產生待確認預覽，不清楚時先追問。
例如：「保留這些零件，改成 10 公分才警告」或「HC-SR04+ 是做什麼的？」。
在 Blueprint／接線／部署階段收到新版時顯示「有新版作品待確認」，不跳頁、不覆寫草稿；Blueprint 移除對話框不會中斷背景 AI 工作。
生成方式也不再需要切換：AI 依當次需求選擇局部修改（保留未修改的外形與配置）或全新造型（不帶前版圖片）；不清楚時先詢問。對話保留脈絡，舊訊息不視為新指令。
設計頁參照最新預覽，沒有預覽時參照已確認作品。任何新版本都先預覽，不覆蓋已確認作品、接線與手動程式。
「載入 Demo 示範」使用內建 Pi 5＋HC-SR04+＋MRD-TFT240 範例，不呼叫雲端；仍需確認才套用。已有未確認預覽時，先詢問是否替換。
「清除對話」先確認，只清聊天紀錄，保留作品、預覽、接線、程式與未送出的文字；AI 工作未完成時禁止清除或載入 Demo。
